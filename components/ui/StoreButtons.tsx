...
const StoreButton = styled.a<{ $disabled?: boolean }>`
  ...
  background: ${({ $disabled }) => ($disabled ? 'transparent' : 'var(--button-primary-bg)')};
  color: ${({ $disabled }) => ($disabled ? 'var(--text-muted)' : 'var(--button-primary-text)')};
...
export function StoreButtons() {
  return (
    <Wrap>
      ...
      <StoreButton as="div" $disabled aria-disabled="true">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
